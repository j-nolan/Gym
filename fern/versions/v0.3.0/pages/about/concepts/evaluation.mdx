...
An **environment** is the executable task interface: the dataset, agent harness, verifier, and state that let an agent attempt a task and receive a score. It defines how the agent interacts with the task and how each attempt is measured.

A **benchmark** is the evaluation target and reporting frame built around one or more environments: the task set, scoring rules, run settings, and aggregate metrics used to compare models or agents. For example, EvalPlus is a benchmark for code generation, while the EvalPlus environment is the concrete NeMo Gym integration that runs tasks and scores attempts.

Use environments when you are talking about runtime behavior and task execution. Use benchmarks when you are talking about comparative results, leaderboards, or the named eval suite being reported.
